...
#[tokio::test]
async fn test_deposit_no_locking_token_2022() -> Result<(), TransportError> {
    let context = TestContext::new().await;
    let addin = &context.addin;
    ...
    let reference_account = context.users[1].token_accounts[2];
    let get_balances = |depot_id| {
        balances(
            &context,
            &registrar,
    ...
    addin
        .deposit(
            ...
            &token_2022_voting_mint,
            &voter2_authority,
            context.users[2].token_accounts[2],
            5,
            1000,
        )
        .await
