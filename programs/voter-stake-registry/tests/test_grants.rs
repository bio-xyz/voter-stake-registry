...
async fn test_grants() -> Result<(), TransportError> {
    ...
    assert_eq!(deposit.allow_clawback, 1);
...
#[tokio::test]
async fn test_grants_token_2022() -> Result<(), TransportError> {
    let context = TestContext::new().await;
    let addin = &context.addin;
    ...
    let grant_authority = &context.users[3].key;
    let grant_funds = context.users[3].token_accounts[2];

    let token_2022_voting_mint = addin
        .configure_voting_mint(
            &registrar,
            &realm_authority,
            payer,
